...
# The `app` import will now correctly use the test DB path from conftest
...
def test_time_entries_date_range_edges(test_db):
    conn = test_db
    entries = [
...
def test_get_daily_note_not_found(test_db):
    resp = client.get("/daily_notes/2025-01-15")
    assert resp.status_code == 200
    assert resp.json() is None


def test_create_daily_note(test_db):
...
def test_get_existing_daily_note(test_db):
...
def test_update_daily_note(test_db):
...
def test_daily_note_special_characters(test_db):
...
def test_daily_note_date_formats(test_db):
    resp = client.put("/daily_notes/2025-12-31", json={"note_content": "End of year"})
    assert resp.status_code == 200


def test_multiple_daily_notes(test_db):
    notes = {
...
def test_daily_note_concurrent_updates(test_db):
    date_str = "2025-03-01"
...
def test_daily_note_isolation_between_dates(test_db):
